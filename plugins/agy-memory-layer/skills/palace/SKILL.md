...
2. **In-Chat Artifact Mode**:
   ...
   - 🗺️ **Mermaid Graph**: Connections between Global User profile, Active Project, and Rules.
...
```mermaid
graph TD
    subgraph GlobalMemory["Global Memory (~/.gemini/memory/global)"]
        HUMAN["👤 human.md<br/>User Profile & Preferences"]
        PERSONA["🤖 persona.md<br/>Agent Identity & Rules"]
    end
    
    subgraph ProjectMemory["Project Memory (~/.gemini/memory/projects/<slug>)"]
        PROJECT["🏗️ project.md<br/>Architecture & Boundaries"]
        RULES["📋 rules.md<br/>Conventions & Linters"]
        LEARNINGS["📖 learnings/<br/>Session Summaries"]
    end
