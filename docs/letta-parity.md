...
| Context compaction is distinct from memory maintenance | `memory-compactor.ts` is read-only Markdown maintenance analysis; progressive middle-truncation (`middleTruncateText`) preserves 30% head and 30% tail in recall and dream extraction; host compaction summaries and recall results are treated as historical evidence under canonical doctrine in `rules/AGENTS.md` and PreInvocation authority stanza, providing bounded model-guided compaction resistance without deterministic host interception | **Implemented boundary, middle-truncation & model-guided doctrine; 8/8 coached baseline plus 4/4 uncoached remediation** ([evidence](#model-guided-authority-host-matrix--2026-09-02)) |
...
| Subagent tools and memory scope are enforced at launch | JSON manifests resolve declarative role/capability intent; `prepareSubagentExecution` wires writer subagents to isolated Git worktrees via `worktree-manager.ts` | **Implemented worktree isolation; host tool confinement remains declarative** |
