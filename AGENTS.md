...
- `package.json` owns current version intent. `plugins/agy-memory-layer/plugin.json` mirrors it, and `palace-generator.ts` dynamically reads `plugin.json` at runtime (not a static version-bump surface).
...
and **108/108 aggregate host-evidence** tests. Final `v1.20.0` release verification
...
coverage at 86.56% lines / 74.75% branches / 90.66% functions. Do not infer
unsupervised readiness from the bounded release evidence.
